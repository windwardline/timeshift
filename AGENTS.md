...
CI gates every PR: lint → typecheck → tests (`test:run` — bare `test` is watch mode) → build, plus `security.yml` (Semgrep, secret scan, dependency scan — required checks; a Headers live job asserts the production security headers on push and a daily cron — scans stay weekly — and they are platform-applied from `vercel.json` and contract-tested by `security-headers.test.ts`). `security-rls.test.ts` is the same shape for the database: it fails CI when a model ships without its `ENABLE ROW LEVEL SECURITY` line, or when a later migration disables RLS or re-grants to `anon`/`authenticated` (see §5). An advisory Claude review runs on every same-repo PR via `claude-review.yml`, which deliberately calls the fleet reusable at `@main` — one merge updates every repo. It activates only when the `CLAUDE_CODE_OAUTH_TOKEN` secret is present — reviews bill the owner's Claude subscription, not Console credits; fork PRs never receive secrets, so they skip it by security design. That first lane is `ci.yml` — pull requests and pushes to `main`, Node 24, `npm ci`, then lint, typecheck, `test:run`, build — and it carries a placeholder `DATABASE_URL` in the job env purely because `npm ci` runs `prisma generate` through `postinstall` and the datasource variable has to exist; nothing ever connects to it, so do not read it as a database. Dependabot's lane is `dependabot-auto-merge.yml`, byte-identical in every fleet repo that takes it: it merges nothing itself — it arms GitHub's native auto-merge (`gh pr merge --squash --auto --delete-branch`) so the `main-requires-green-ci` ruleset stays the only thing that decides whether a merge happens, and because `--auto` degrades to an immediate merge where no gate exists it first asserts `allow_auto_merge` and at least one required check on the base branch, holding rather than arming if either is missing. It runs only for `dependabot[bot]` pull requests raised on this repo under the `windwardline` owner (`claude-review.yml` skips runs with that actor, so bot PRs get no advisory review), and it holds for a human — withdrawing any auto-merge it armed on an earlier push — on a major bump (labelled `deferred-major` first, so a hold for some other reason cannot cost it the label), a `no-automerge` label, a release that changed maintainers, a pre-1.0 package, or Dependabot metadata that came back empty or unverifiable. It prefers a GitHub App token minted from `FLEET_AUTOMERGE_APP_ID`/`FLEET_AUTOMERGE_PRIVATE_KEY`, which have to be **Dependabot** secrets — a Dependabot-triggered run cannot read Actions secrets and gets an empty string rather than an error — and degrades to `GITHUB_TOKEN` when they are absent, so the run summary names which credential was used: a push attributed to `GITHUB_TOKEN` creates no workflow run at all, and on that fallback path an auto-merged commit fires none of `security.yml`'s push jobs, `Headers live` included. Its job id carries no `name:`, so the check renders exactly `dependabot-auto-merge` — the string the fleet conformance audit excludes, because **this job must never become a required check**.
