# AGENTS.md — TimeShift Context, Guardrails & Rules
...
CI gates every PR: lint → typecheck → tests (`test:run` — bare `test` is watch mode) → build, plus `security.yml`. Semgrep CE and Secret scan run on pull requests, pushes to `main`, and the weekly schedule. Dependency scan runs on every trigger, including the daily schedule; Headers live runs on pushes and that daily schedule. The headers come from `vercel.json` and are contract-tested by `security-headers.test.ts`. The required Secret scan check also executes the pinned `verify-action-pins` step, so a mutable third-party `uses:` ref or a SHA/tag-comment mismatch blocks the merge under Secret scan. `security-rls.test.ts` is the same shape for the database: it fails CI when a model ships without its `ENABLE ROW LEVEL SECURITY` line, or when a later migration disables RLS or re-grants to `anon`/`authenticated` (see §5).

An advisory Claude review runs on eligible same-repo PR events when `github.actor` — the original event actor — is not `dependabot[bot]`. `claude-review.yml` deliberately calls the fleet reusable at `@main`, so one merge updates every repo. It activates only when the `CLAUDE_CODE_OAUTH_TOKEN` secret is present; forks never receive secrets and skip by security design. Reviews bill the owner's Claude subscription, not Console credits. The first CI lane is `ci.yml`: pull requests and pushes to `main`, Node 24, `npm ci`, then lint, typecheck, `test:run`, and build. Its placeholder `DATABASE_URL` exists only because `npm ci` runs `prisma generate` through `postinstall`; nothing connects to it.

Dependabot's `dependabot-auto-merge.yml` is byte-identical in every fleet repo that takes it. It merges nothing itself: it arms GitHub's native auto-merge (`gh pr merge --squash --auto --delete-branch`) so `main-requires-green-ci` decides whether a merge happens. Because `--auto` becomes an immediate merge when no gate exists, the lane first requires `allow_auto_merge` and at least one required check on the base branch. It runs only for `dependabot[bot]` pull requests raised in this repo under the `windwardline` owner. It holds for a human — withdrawing auto-merge armed on an earlier push — when the gate is absent, a major bump, an unrecognised update type, a `no-automerge` label, changed maintainers, a pre-1.0 package, or empty or unverifiable metadata. When the update-type decision is reached, a major receives the `deferred-major` label before it is held.

Dependabot groups npm production dependencies, npm development dependencies, and GitHub Actions updates. `fetch-metadata` reports the highest semver change for the entire grouped PR, so one held member holds the group; arming and holding are per grouped PR, not per package or action. The lane prefers a GitHub App token minted from `FLEET_AUTOMERGE_APP_ID`/`FLEET_AUTOMERGE_PRIVATE_KEY`, which must be **Dependabot** secrets. A Dependabot-triggered run cannot read Actions secrets and receives an empty string instead. The lane falls back to `GITHUB_TOKEN` and names the credential in its summary. A merge attributed to that fallback token creates no push workflow run, so none of `security.yml`'s push jobs fire. The job has no `name:`, so its check is exactly `dependabot-auto-merge`; it must never become required.
